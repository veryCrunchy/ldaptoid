...
export class StructuredLogger {
  ...
  private sensitiveFields = new Set([
    ...
    'authorization', 'x-api-key', 'client_secret', 'access_token'
  ]);
  ...
  debug(message: string, data?: Record<string, unknown>, context?: { requestId?: string; userId?: string; clientIp?: string }): void {
    if (this.config.level <= LogLevel.DEBUG) {
      this.log(LogLevel.DEBUG, message, data, context);
    }
  }

  info(message: string, data?: Record<string, unknown>, context?: { requestId?: string; userId?: string; clientIp?: string }): void {
    if (this.config.level <= LogLevel.INFO) {
      this.log(LogLevel.INFO, message, data, context);
    }
  }

  warn(message: string, data?: Record<string, unknown>, context?: { requestId?: string; userId?: string; clientIp?: string }): void {
    if (this.config.level <= LogLevel.WARN) {
      this.log(LogLevel.WARN, message, data, context);
    }
  }

  error(message: string, error?: Error | unknown, data?: Record<string, unknown>, context?: { requestId?: string; userId?: string; clientIp?: string }): void {
  ...
  fatal(message: string, error?: Error | unknown, data?: Record<string, unknown>, context?: { requestId?: string; userId?: string; clientIp?: string }): void {
    const errorData = error instanceof Error ? {
      name: error.name,
      message: error.message,
      ...(error.stack ? { stack: error.stack } : {})
  ...
  private log(level: LogLevel, message: string, data?: Record<string, unknown>, context?: { requestId?: string; userId?: string; clientIp?: string }, error?: { name: string; message: string; stack?: string }): void {
